...
import { publicBucket } from "./model/infrastructure";
import { Worker, R2Bucket, Container, AccountApiToken, Workflow } from "alchemy/cloudflare";
...
// Define Workflow binding for async transcoding
const transcodeWorkflow = Workflow("transcode-workflow", {
  workflowName: "transcode-workflow",
  className: "TranscodeWorkflow",
});
...
export const worker = await Worker("recaller-worker", {
  ...
  bindings: {
    ...
    TRANSCODE_WORKFLOW: transcodeWorkflow,
...
// Write the Worker URL to modal_runner/.env for Python to consume
try {
  const modalRunnerEnvPath = import.meta.dir + "/../modal_runner/.env";
  await Bun.file(modalRunnerEnvPath).write(
    `PERFECT_READS_WORKER_URL=${worker.url}\n`
  );
  console.log(`Wrote PERFECT_READS_WORKER_URL to: ${modalRunnerEnvPath}`);
} catch (e) {
  console.log("Failed to write modal_runner/.env:", e);
}
